...
class MCMCGrader(object):
    ...
    def submit_simulation_trajectory(self, alice_trajectory, bob_trajectory):
        self.submit_part('pn017', '{}  {}'.format(
            self.ravel_output(alice_trajectory[0]),
            self.ravel_output(alice_trajectory[1])
        ))
        self.submit_part('UUbsF', '{}  {}'.format(
            self.ravel_output(bob_trajectory[0]),
            self.ravel_output(bob_trajectory[1])
        ))

    def submit_simulation_mean(self, alice_price, bob_price):
        self.submit_part(
            'FFmXD', str(self.ravel_output(alice_price))
        )
        self.submit_part(
            'uWPFR', str(self.ravel_output(bob_price))
        )

    def submit_simulation_correlation(self, alice_bob_correlation):
        self.submit_part(
            'nkkem', str(self.ravel_output(alice_bob_correlation))
        )

    def submit_simulation_depends(self, answer):
        self.submit_part('dyuVW', answer)

    def submit_pymc_map_estimates(self, beta_age_coefficient,
                                  beta_education_coefficient):
        self.submit_part(
            'r1VVR', str(self.ravel_output(beta_age_coefficient))
        )
        self.submit_part(
            '5wFjO', str(self.ravel_output(beta_education_coefficient))
        )

    def submit_pymc_odds_ratio_interval(self, odds_ratio_lower_bound,
                                        odds_ratio_upper_bound):
        self.submit_part(
            'sn9Lu', str(self.ravel_output(odds_ratio_lower_bound))
        )
        self.submit_part(
            'JHRF9', str(self.ravel_output(odds_ratio_upper_bound))
        )
