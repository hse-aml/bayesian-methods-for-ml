...
class VAEGrader(object):
    ...
    def submit_best_val_loss(self, hist):
        self.submit_part(
            'NRPCA', str(self.ravel_output(hist.history['val_loss'][-1]))
        )
    ...
    def submit_hallucinating(self, sess, sampled_im_mean):
        try:
            ...
            raise e
        self.submit_part(
            'JEmpp', str(self.ravel_output(np.mean(imgs)))
        )
        var_per_channel = np.var(imgs, axis=0)
        self.submit_part(
            '3K3IB', str(self.ravel_output(np.max(var_per_channel)))
        )

    def submit_conditional_hallucinating(self, sess,
                                         conditional_sampled_im_mean):
        ...
        self.submit_part(
            'tYD01', str(self.ravel_output(np.mean(diff)))
        )
        var_per_channel = np.var(diff, axis=0)
        self.submit_part(
            'CaofU', str(self.ravel_output(np.max(var_per_channel)))
        )
